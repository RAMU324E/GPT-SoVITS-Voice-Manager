# GPT-SoVITS Voice Manager

一个基于 [RVC-Boss/GPT-SoVITS](https://github.com/RVC-Boss/GPT-SoVITS) 的本地语音管理与 TTS API 平台，增加了适合日常使用的网页前端、音色注册、按需模型切换和“开源阅读”接入。

> 本仓库只发布源码与莱莎四音色配置示例，不包含模型权重、参考音频、预训练模型、Python runtime、生成音频或任何 Secret。克隆后需要自行准备这些本地资产。

## 增加的功能

- **管理页 `/admin`**：扫描模型目录、注册/删除正式音色、修改配置、设置默认 `auto / zh / ja` 音色。
- **合成页 `/studio`**：选择音色、输入文本、试听、下载、保存结果和测速。
- **标准 TTS API**：`POST /v1/tts`，JSON 请求，直接返回 `audio/wav` 或 `audio/mpeg`。
- **音色列表与状态**：`GET /v1/voices`、`GET /health`、`GET /v1/active-voice`。
- **按需模型切换**：同一时间只让一个 GPT-SoVITS 后端占用显存，适合显存有限的本地设备。
- **音色自动发现**：从 `.ckpt`、`.pth`、参考音频和 `meta.json` / `prompt.txt` 生成正式 YAML 配置。
- **v2Pro / v2ProPlus**：扩展模型版本扫描与推理支持。
- **阅读 App 模板**：管理页为每个音色生成可复制的自定义朗读 URL。
...
|---|---|---|---|
| `ja_ryza` | 莱莎 JP | 日语 | 普通 |
| `ja_ryza_asmr` | 莱莎 JP（ASMR） | 日语 | 耳语 |
| `zh_ryza` | 莱莎 CN | 中文 | 普通 |
| `zh_ryza_asmr` | 莱莎 CN（ASMR） | 中文 | 耳语 |

以下内容用于说明 `voice_id` 的配置和 API 调用方式。仓库不包含这些 ID 对应的模型权重与参考音频；实际部署时请在 `local_assets/voices/` 中自行放置并注册。
...
兼容路径：`/`、`/tts`、`/v1/tts`、`/v1/audio/speech`。请求也兼容 `input`、`voice`、`text_lang`、`media_type`、`speed` 等字段别名。
...
```text
<项目目录>/runtime/                         # 可选的本地 Python 运行环境
<项目目录>/GPT_SoVITS/pretrained_models/   # GPT-SoVITS 预训练模型
<项目目录>/local_assets/voices/            # 音色权重和参考音频
```
...
完整的本地平台教程见 [README_LOCAL_TTS.md](./README_LOCAL_TTS.md)，维护与注册说明见 [LOCAL_TTS_MAINTENANCE.md](./LOCAL_TTS_MAINTENANCE.md)。
...
- Studio：<http://127.0.0.1:9881/studio>
- 管理页：<http://127.0.0.1:9881/admin>
- API 文档：<http://127.0.0.1:9881/docs>

## 许可与内容说明

- GPT-SoVITS 上游代码遵循仓库内的 MIT License；第三方组件保留各自许可证。
- 本仓库不分发角色模型、训练权重或参考音频，也不授予这些未包含资产的再分发权。
- 角色名称、声音、素材及相关权利归各自权利人；请仅在获得授权和符合当地法律、平台条款的范围内使用。

---

## Upstream GPT-SoVITS README
